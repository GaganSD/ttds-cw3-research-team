...
class MongoDBClient():
    ...
    def remove_doc_index(self, term: str, doc_id: str):
        """
        The method to remove a document from the index of a term
        WARNING: DO OVERWRITE at [term][doc_id] level. (pos and len)
        Parameters:
            term - The term to be updated.
            doc_id - The document to be removed from the index
        """
        ...
        cur_chain = [term]
        if hq == None:
            print("Error this term does not exist in the index!")
        else:
            cur_size = hq["doc_count"] 
            cur_chain = hq["chain"]
        
        new_cur_size = cur_size
        
        for temp_chain in cur_chain:
            temp_table = cur_table.find_one({"_id": temp_chain})
            temp_docs = temp_table['docs']
